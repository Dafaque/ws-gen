...
func (ch *connectionHandler) rloop() {
    for {
		...
		if err := mapper.Read(ch); err != nil {
            ch.close(websocket.CloseProtocolError, err.Error())
        }
	}
}
...
func (ch *connectionHandler) CloseHandler(code int, reason string) error {
	...
	return ch.close(websocket.CloseNormalClosure, "")
}

func (ch *connectionHandler) close(code int, text string) error {
    return ch.conn.WriteControl(
